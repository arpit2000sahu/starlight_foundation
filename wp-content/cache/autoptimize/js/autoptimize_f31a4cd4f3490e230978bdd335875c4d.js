...
var map;
var mum = { lat: 18.9854293, lng: 72.8312272 },
  del = { lat: 28.5179549, lng: 77.1972357 },
  bglr = { lat: 12.9974674, lng: 77.6164133 },
  kol = { lat: 22.500872, lng: 88.3923885 };
$(document).ready(function () {
  changeCityDetails();
  checkContactType();
});
function checkContactType() {
  ...
  if (contactType !== "undefined" && contactType != "N/A") {
    var ele,
      optionValue,
      transformedOptionVal,
      subjectChanged = false;
    $(".contact-subject option").each(function () {
      ele = $(this);
      optionValue = ele.val();
      transformedOptionVal = optionValue
        .replace(/[^a-z0-9\s]/gi, "")
        .replace(/[_\s]/g, "-")
        .toLowerCase();
      if (
        transformedOptionVal.indexOf(contactType) != -1 &&
        subjectChanged == false
      ) {
...
function changeCityDetails() {
  ...
  $(document).on("click", ".contact-locations-tab-wrapper span", function () {
    ...
    newLocation = $(
      ".contact-locations-info-wrapper .contact-locations-info:nth-child(" +
        locationIndex +
        ")"
    );
...
function initMap() {
  ...
  var icons = { info: { icon: iconBase } };
  var features = [
    { position: mum, type: "info" },
    { position: del, type: "info" },
    { position: bglr, type: "info" },
    { position: kol, type: "info" },
  ];
